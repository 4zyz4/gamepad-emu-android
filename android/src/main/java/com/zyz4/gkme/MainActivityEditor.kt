...
internal fun MainActivity.addControl(entry: CtrlEntry) {
    ...
    val view: View = when {
        ...
        entry.baseId.startsWith("btnMouse") -> com.zyz4.gkme.view.RotatableButton(a).apply {
            this.id = View.generateViewId(); tag = id
...
@SuppressLint("ClickableViewAccessibility")
internal fun MainActivity.setupKeyboardTouchHandler(view: View, keyCode: Int, pos: com.zyz4.gkme.model.ButtonPosition) {
    val a = this
...
internal fun MainActivity.createStandardControlView(pos: ButtonPosition) {
    ...
    val view: View = when {
        pos.linearTriggerEnabled && baseId in triggerBaseIds -> {
            com.zyz4.gkme.view.LinearTriggerView(a).apply {
                id = View.generateViewId(); tag = pos.id
                text = when (mode) {
                    DisplayMode.XBOX -> "LT"
        ...
        baseId.startsWith("btnMouse") -> com.zyz4.gkme.view.RotatableButton(a).apply {
            id = View.generateViewId(); tag = pos.id
